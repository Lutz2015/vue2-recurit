...
exports.install = function (Vue, options) {
    ...
    Popup.prototype.show = function (opt) {
        ...
        // 右上角叉叉
        var closeIcon = opt.closeIcon && '<div class="close-icon"><i class="icon-cross"></i></div>' || '';
        // 输出标题
        var title = opt.title && '<div class="title">' + opt.title + '</div>' || '';
        // 输出消息
        var msg = opt.msg && '<div class="msg">' + opt.msg + '</div>' || '';
        // 输出按钮组
        var btn = '<div class="btn row border-tp-e1">';
        for (var i = 0, len = opt.btn.length; i < len; i++) {
            btn += '<a class="popup-btn col border-rt-e1" data-index="' + i + '">' + opt.btn[i].name + '</a>';
        }
        btn += '</div>';
        ele.addEventListener('click', function (e) {
            ...
            if (e.target && e.target.className.match('close-icon')) {
                // opt.closeCb && opt.closeCb.call(self);
                self.close();
            }
        });
